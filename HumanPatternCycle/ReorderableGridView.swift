...
class ReorderableView: UIView, UIGestureRecognizerDelegate {
    ...
    override init(frame: CGRect) {
        ...
        let oneTap = UITapGestureRecognizer (target: self, action: "oneTapped:") // Для одного нажатия определяем действие ниже:
    ...
    func oneTapped (gesture: UITapGestureRecognizer) { // если один раз тапнули
        if self.tag == 999 {  // Если на кнопку ДОБАВИТЬ
         secondController.addNewMood()
            
        } else {
        print("--one click \(self.tag)")
        AddNewCont.addRecord(self.tag) // Передадим номер выбранного элемента
        }
    }
